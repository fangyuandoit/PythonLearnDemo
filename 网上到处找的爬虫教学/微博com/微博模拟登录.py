...
def textPassword2encodePassword(textPassword,servertime,nonce,pubkey):
    context.execute(sina_js)
    encodePassword = context.getpass(textPassword,servertime,nonce,pubkey)
    return encodePassword;


#字符串转base64
def string2Base64(sourceMessage):
    encodeByte = base64.b64encode('15828313741'.encode('utf-8'))
...
def  prelogin():
    ...
    url="https://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su="+string2Base64(username)+"%3D&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.19)&_="+str(currTime)+"";

    response = requests.get(url)
    cookies = response.cookies.get_dict()
    text = response.text
    text = text.replace("sinaSSOController.preloginCallBack(","")
    text = text.replace(")","")
    jsonObject = json.loads(text)
    # print(jsonObject)
    retcode = jsonObject["retcode"]
    servertime = jsonObject["servertime"]
    pcid = jsonObject["pcid"]
    nonce = jsonObject["nonce"]
    pubkey = jsonObject["pubkey"]
    rsakv = jsonObject["rsakv"]
    is_openlock = jsonObject["is_openlock"]
    showpin = jsonObject["showpin"]
    exectime = jsonObject["exectime"]

    p =preLoginAndLoginParam()
    p.cookies =cookies;
    p.retcode =retcode;
...
def account(p):
     ...
     headers={
         ...
         "Referer":"https://weibo.com/login.php?url=https%3A%2F%2Fweibo.com%2Fu%2F1879069995%2Fhome%3Fwvr%3D5",
...
p = prelogin()
encode_password = textPassword2encodePassword(password,p.servertime,p.nonce,p.pubkey)
p.sp=encode_password
p.su = string2Base64(username)
cookies = account(p)

text = requests.post("https://weibo.com/xiaogang2099", cookies=cookies).text
print(text)
